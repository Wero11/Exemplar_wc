...
function themefusion_pagination($pages = '', $range = 2)
{
    global $data;
     ...
     if(1 != $pages)
     {
         ...
         if($paged > 1) echo "<a class='pagination-prev' href='".get_pagenum_link($paged - 1)."'><span class='page-prev'></span>".__('Previous', 'Exemplar')."</a>";
         ...
         if ($paged < $pages) echo "<a class='pagination-next' href='".get_pagenum_link($paged + 1)."'>".__('Next', 'Exemplar')."<span class='page-next'></span></a>";  
...
function themefusion_breadcrumb() {
		...
		if($c_pageID == -9999)
		{
			...
			echo "<li><a href='".tribe_get_events_link()."'>".__("Events", "Exemplar")."</a></li>";
		}

        if (is_category() && !is_singular('avada_portfolio') && !is_singular('avada_blog') && !is_singular('avada_news') && !is_singular('event') && !is_singular('tribe_events')) {
		...
		if(is_singular('avada_blog')) {
            echo get_the_term_list($post->ID, 'blog_category', '<li>', '&nbsp;/&nbsp;&nbsp;', '</li>');  
            echo '<li>'.get_the_title().'</li>'; 
        }

		if(is_singular('avada_news')) {
            echo get_the_term_list($post->ID, 'news_category', '<li>', '&nbsp;/&nbsp;&nbsp;', '</li>');  
            echo '<li>'.get_the_title().'</li>'; 
        }

		if(is_singular('tribe_events')) {
			$qry = "SELECT ID from wp_posts WHERE post_name='".$query_var["name"]."'";
			$post_id = $wpdb->get_var($wpdb->prepare($qry));
        ...
        if(is_single() && !is_singular('avada_portfolio') && !is_singular('avada_blog') && !is_singular('avada_news') && !is_singular('event') && !is_singular('tribe_events') ) {
        ...
        if(is_404()){ echo '<li>'.__("404 - Page not Found", 'Exemplar').'</li>'; }
        if(is_search()){ echo '<li>'.__("Search", 'Exemplar').'</li>'; }
