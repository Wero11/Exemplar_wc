...
class PyreThemeFrameworkMetaboxes {
	...
	public function add_meta_boxes()
	{
		...
		$this->add_meta_box('page_options', 'Page Options', 'avada_blog');
		$this->add_meta_box('page_options', 'Page Options', 'avada_news');
	...
	public function select_group($id, $label, $options, $desc = '')
	{
		global $post;
		...
		$html .= '<div class="pyre_metabox_field">';
			...
			$html .= '</label>';
			$html .= '<div class="field">';
				$html .= '<select id="pyre_' . $id . '" name="pyre_' . $id . '[]" multiple size="8">';
				$before_parent = 0;
				$indent = "";
				foreach($options as $key => $option) {
					if( $option["parent"] == 0 )
					...
					if( $option["parent"] != $before_parent )
					{
						$indent .= "&nbsp;&nbsp;&nbsp;&nbsp;";
						$before_parent = $option["parent"];
					}

					$selected_val = get_post_meta($post->ID, 'pyre_' . $id, true);

					if( $selected_val != 0 && in_array($option["val"], get_post_meta($post->ID, 'pyre_' . $id, true) )) {
						$selected = 'selected="selected"';
					} else {
						$selected = '';
					}
					
					$html .= '<option ' . $selected . 'value="' . $option["val"] . '">' . $indent.$option["name"] . '</option>';
				}
				$html .= '</select>';
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
