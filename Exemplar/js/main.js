jQuery(window).load(function() {
	...
	var admin_bar_show = false;
	var nav_height = jQuery("#small-nav").outerHeight();
	
	if( jQuery(".announcement-wrapper").length > 0 ){
		header_height = header_height + jQuery(".announcement-wrapper").outerHeight();
	}

	if( jQuery(".header-social").length > 0 ){
		header_height = header_height + jQuery(".header-social").outerHeight();
	}

	if( jQuery("#wpadminbar").length > 0)
		admin_bar_show = true;

	var empty_div = "<div class='empty_div' style='width:100%; height:"+nav_height+"px; display:block;' ></div>";
	
	var windowPos = jQuery(window).scrollTop();
	if(windowPos >= header_height ) {
		jQuery('#small-nav').addClass("fixed");
		if(admin_bar_show)
			jQuery(".fixed").css("top","28px");

		if( !jQuery(".empty_div").length )
		{
			jQuery("#header").after(empty_div);
		}
	}
...
jQuery(document).ready(function($) {
	...
	jQuery(".tribe-events-present").addClass("tribe-mini-calendar-today");
});