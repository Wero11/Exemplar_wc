...
add_shortcode('tagline_box', 'shortcode_tagline_box');
	function shortcode_tagline_box($atts, $content = null) {
		...
		elseif($atts['type']==2)
		{
			$str .= '<section class="reading-box" style="background-color: #f6f6f6 !important;border-width: 1px;border-color: #f6f6f6!important; border-top-width: 3px !important;border-top-color: '.$data['primary_color'].'!important;border-style: solid;">';
				if($atts['link'] && $atts['button']):
				$str .= '<a href="'.$atts['link'].'" target="'.$atts['linktarget'].'" class="continue button large green">'.$atts['button'].'</a>';
				endif;
				if($atts['title']):
...
function shortcode_recent_posts($atts, $content = null) {
	...
	} elseif (!empty($atts['post_type']) && $atts['post_type']){
		$recent_posts = new WP_Query('showposts='.$atts['number_posts'].'&post_type='.$atts['post_type']);
	}
	else {
	...
	$html .= '<li><a href="'.get_permalink(get_the_ID()).'">'.get_comments_number(get_the_ID()).' '.__('Comments', 'Exemplar').'</a></li>';
	endif;
	// JCH
	if (!empty($atts['post_type']) && $atts['post_type']){
		if($atts['post_type'] == "avada_news")
			$term = 'news_category';
		elseif($atts['post_type'] == "avada_blog")
			$term = 'blog_category';

		$selected_terms = get_the_terms( get_the_ID(), $term );
	...
	else
	{
		...
		foreach($post_categories as $c){
			$cat = get_category( $c );
			$selected_terms_list[] = $cat->name;
		}
	}
	$html .= '<li><a href="'.get_permalink(get_the_ID()).'">'.implode(", ", $selected_terms_list).'</a></li>';// END Modify
