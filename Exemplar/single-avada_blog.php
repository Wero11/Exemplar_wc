<?php get_header(); ?>
	...
	$selected_terms = get_the_terms( $post->ID, 'blog_category' );
	$selected_terms_list = array();
	foreach($selected_terms as $sterm){
		$selected_terms_list[] = $sterm->slug;
	}
	?>
	<div class="sidebar" style="<?php echo $sidebar_css; ?>">
		<div class="widget news-filter-nav">
			<h3><?php echo __("Blogs Menu", "Exemplar")?></h3>
		...
		<?php
			...
			<ul class="side-nav">
				<?php wp_reset_query(); ?>
			<!--	<?php
					$args = array( "post_type"=>"avada_blog", "posts_per_page"=>-1 , "orderby"=>"date", "order"=>"desc" , "post_status"=>"publish" );
					query_posts( $args );
					...
					if(have_posts() ){
						while ( have_posts() ) : the_post();
							...
							if( $current_post_id == $post_id ){
								$selected_class = "current_page_item";
								$link = "#";
							}
							else
							{
					...
					?>
						<li class="<?php echo $selected_class?>"><a href="<?php echo $link; ?>"><?php echo get_the_title($post_id); ?></a></li>
						<?php endwhile; ?>
				<?php } ?>-->
				<?php
				$i = 0;
				...
				{
					if( in_array($term->slug, $selected_terms_list) ) 
	...
	<div id="content" style="<?php echo $content_css; ?>">
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('post'); ?>>
			...
			<div class="share-box">
				<h4><?php echo __('Share This Story, Choose Your Platform!', 'Exemplar'); ?></h4>
			...
			<div class="about-author">
				<div class="title"><h2><?php echo __('About the Author:', 'Exemplar'); ?> <?php the_author_posts_link(); ?></h2></div>
			...
			<div class="single-navigation clearfix">
				<div class="backtolist">
					<a href="<?php echo get_bloginfo("url")?>/blogs"><?php echo __("Back to Blog List", "Exemplar")?></a>
				</div>
				<?php previous_post_link('%link', __('Previous Blog Post', 'Exemplar')); ?>
				<?php next_post_link('%link', __('Next Blog Post', 'Exemplar')); ?>
			</div>
			...
			<div class="related-posts">
				<div class="title"><h2><?php echo __('Related Posts', 'Exemplar'); ?></h2></div>
	...
	<div class="sidebar" style="<?php echo $sidebar_right_css; ?>">
		<div class="widget ">
			<h3><?php echo __("Recent Posts", "Exemplar")?></h3>
			<?php
			$args = array( "post_type"=>"avada_blog", "tax_query"=>array( array("taxonomy"=> "blog_category", "field"=>"slug", "terms"=>$selected_terms_list) ), "posts_per_page"=>-1, "post_status"=>"publish" );
			query_posts($args);
			$current_post_id = $post->ID;
			if (have_posts()) : ?>
			<ul class="">
			...
			<?php else :?>
			<?php echo __("There isn't blogs under the current category","Exemplar"); ?>
			<?php endif; ?>
		</div>
	</div>
<?php get_footer(); ?>